...
export async function checkB102({evaluate,route,command,poll,screenshot,delay,importSynthetic}){
 ...
 await evaluate(`(async()=>{
  ...
  s.moduleProgress.B102={selfChecked:true,date:'2026-09-22'};s.drafts.B102='Synthetic original notes for B102.';
  s.cards['B102-v1']=reviewCard(null,'good',Date.UTC(2026,8,22));
  s.navigation.pages['module/B102']={scroll:0,focus:'drill1',fields:{drill0:'were',drill1:'had\\nOriginal answer',drill2:'do not'},details:[]};
  window.__b102Import=JSON.stringify(s);
 })()`);
 const original=await evaluate('window.__b102Import');await importSynthetic('window.__b102Import');
 await route('module/B102','#legacy-practice');
 ...
 assert.equal(await evaluate('document.querySelector("#draft").value'),'Synthetic original notes for B102.');
 ...
 await route('home','.hero');await route('module/B102','#legacy-practice');
 await command('Page.reload');await poll(()=>evaluate('document.querySelector("#drill2")?.value==="do not"'),'B102 archive reload');
 await screenshot('b102-topic-desktop.png');
 const units=await evaluate(`(async()=>{const {subtopics}=await import('/data/course.mjs');return subtopics.filter(u=>u.topic==='B102')})()`);
 for(const u of units){
  ...
  for(const b of u.banks){
   ...
   if(b.kind==='listening'&&u.id==='B102-hypotheses'){
 ...
 for(const [id,count,query] of [['hypothetical-conditions',29,'unless'],['wishes-possibility',32,'wish']]){
  ...
  await poll(()=>evaluate(`document.querySelector('#reference-search')?.value===${JSON.stringify(query)}`),'B102 reference query restored');
  assert.equal(await evaluate('document.querySelectorAll("#reference-rows tr").length'),filtered);
  await fill('#reference-search','');await screenshot('b102-'+id+'-desktop.png');
 }
 await command('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
 for(const [path,selector,name] of [['module/B102','#legacy-practice','topic'],['unit/B102-possibility/production','#check-bank','writing'],['unit/B102-wishes/reading','#check-bank','reading'],['references/hypothetical-conditions','#reference-search','reference'],['references/wishes-possibility','#reference-search','wishes-reference']]){
  await route(path,selector);assert(await evaluate('document.documentElement.scrollWidth<=window.innerWidth'),'B102 mobile overflow '+path);
  ...
  await screenshot('b102-'+name+'-mobile.png');
 }
 await evaluate(`(async()=>{const {validateState}=await import('/engine.mjs');const s=JSON.parse(localStorage.getItem('english-training-v1'));validateState(s);const old=JSON.parse(${JSON.stringify(original)});if(JSON.stringify(s.cards)!==JSON.stringify(old.cards))throw Error('B102 changed SRS');if(s.navigation.pages['module/B102'].fields.drill1!==old.navigation.pages['module/B102'].fields.drill1)throw Error('Lost B102 old answer');})()`);
 console.log('B102 smoke passed: three units, 22 banks, nested navigation, multiline exams, pending reviews, old archive/SRS, two references and mobile layout.');
}
