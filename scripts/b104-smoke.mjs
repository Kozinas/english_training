...
export async function checkB104({evaluate,route,command,poll,screenshot,delay,importSynthetic}){
 ...
 await evaluate(`(async()=>{
  ...
  s.moduleProgress.B104={selfChecked:true,date:'2026-09-22'};s.drafts.B104='Synthetic original notes for B104.';
  s.cards['B104-v1']=reviewCard(null,'good',Date.UTC(2026,8,22));
  s.navigation.pages['module/B104']={scroll:0,focus:'drill1',fields:{drill0:'told',drill1:'is\\nOriginal answer',drill2:'don’t'},details:[]};
  window.__b104Import=JSON.stringify(s);
 })()`);
 const original=await evaluate('window.__b104Import');await importSynthetic('window.__b104Import');
 await route('module/B104','#legacy-practice');
 assert.equal(await evaluate('document.querySelectorAll(".unit-card").length'),3);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").max'),305);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").value'),0,'no inherited new progress');
 assert.equal(await evaluate('document.querySelector("#draft").value'),'Synthetic original notes for B104.');
 ...
 await route('home','.hero');await route('module/B104','#legacy-practice');
 await command('Page.reload');await poll(()=>evaluate('document.querySelector("#drill2")?.value==="don’t"'),'B104 archive reload');
 await screenshot('b104-topic-desktop.png');
 const units=await evaluate(`(async()=>{const {subtopics}=await import('/data/course.mjs');return subtopics.filter(u=>u.topic==='B104')})()`);
 for(const u of units){
  ...
  for(const b of u.banks){
   ...
   if(b.kind==='listening'&&u.id==='B104-statements'){
 ...
 for(const [id,count,query] of [['reported-speech',32,'must'],['questions-tags',36,'whether']]){
  ...
  await poll(()=>evaluate(`document.querySelector('#reference-search')?.value===${JSON.stringify(query)}`),'B104 reference query restored');
  assert.equal(await evaluate('document.querySelectorAll("#reference-rows tr").length'),filtered);
  await fill('#reference-search','');await screenshot('b104-'+id+'-desktop.png');
 }
 await command('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
 for(const [path,selector,name] of [['module/B104','#legacy-practice','topic'],['unit/B104-tags/production','#check-bank','writing'],['unit/B104-questions/reading','#check-bank','reading'],['references/reported-speech','#reference-search','reference'],['references/questions-tags','#reference-search','services-reference']]){
  await route(path,selector);assert(await evaluate('document.documentElement.scrollWidth<=window.innerWidth'),'B104 mobile overflow '+path);
  ...
  await screenshot('b104-'+name+'-mobile.png');
 }
 await evaluate(`(async()=>{const {validateState}=await import('/engine.mjs');const s=JSON.parse(localStorage.getItem('english-training-v1'));validateState(s);const old=JSON.parse(${JSON.stringify(original)});if(JSON.stringify(s.cards)!==JSON.stringify(old.cards))throw Error('B104 changed SRS');if(s.navigation.pages['module/B104'].fields.drill1!==old.navigation.pages['module/B104'].fields.drill1)throw Error('Lost B104 old answer');})()`);
 console.log('B104 smoke passed: three units, 24 banks, nested navigation, multiline exams, pending reviews, old archive/SRS, two references and mobile layout.');
}
