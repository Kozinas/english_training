...
export async function checkA204({evaluate,route,command,poll,screenshot,delay,importSynthetic}){
 ...
 await evaluate(`(async()=>{
  ...
  s.moduleProgress.A204={selfChecked:true,date:'2026-09-22'};s.drafts.A204='Synthetic original notes for A204.';
  s.cards['A204-v1']=reviewCard(null,'good',Date.UTC(2026,8,22));
  s.navigation.pages['module/A204']={scroll:0,focus:'drill1',fields:{drill0:'was',drill1:'who\\nOriginal answer',drill2:'because'},details:[]};
  window.__a204Import=JSON.stringify(s);
 })()`);
 const original=await evaluate('window.__a204Import');await importSynthetic('window.__a204Import');
 await route('module/A204','#legacy-practice');
 ...
 assert.equal(await evaluate('document.querySelector("#draft").value'),'Synthetic original notes for A204.');
 assert.equal(await evaluate('document.querySelector("#drill1").value'),'who\nOriginal answer');
 ...
 await route('home','.hero');await route('module/A204','#legacy-practice');
 await command('Page.reload');await poll(()=>evaluate('document.querySelector("#drill2")?.value==="because"'),'A204 archive reload');
 await screenshot('a204-topic-desktop.png');
 const units=await evaluate(`(async()=>{const {subtopics}=await import('/data/course.mjs');return subtopics.filter(u=>u.topic==='A204')})()`);
 for(const u of units){
  ...
  for(const b of u.banks){
   ...
   if(b.kind==='listening'&&u.id==='A204-continuous'){
 ...
 for(const [id,count,query] of [['past-narrative',27,'while'],['relative-clauses',24,'whose']]){
  ...
  await poll(()=>evaluate(`document.querySelector('#reference-search')?.value===${JSON.stringify(query)}`),'A204 reference query restored');
  assert.equal(await evaluate('document.querySelectorAll("#reference-rows tr").length'),filtered);
  await fill('#reference-search','');await screenshot('a204-'+id+'-desktop.png');
 }
 await command('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
 for(const [path,selector,name] of [['module/A204','#legacy-practice','topic'],['unit/A204-relative/production','#check-bank','writing'],['unit/A204-story/reading','#check-bank','reading'],['references/past-narrative','#reference-search','reference'],['references/relative-clauses','#reference-search','relative-reference']]){
  await route(path,selector);assert(await evaluate('document.documentElement.scrollWidth<=window.innerWidth'),'A204 mobile overflow '+path);
  ...
  await screenshot('a204-'+name+'-mobile.png');
 }
 await evaluate(`(async()=>{const {validateState}=await import('/engine.mjs');const s=JSON.parse(localStorage.getItem('english-training-v1'));validateState(s);const old=JSON.parse(${JSON.stringify(original)});if(JSON.stringify(s.cards)!==JSON.stringify(old.cards))throw Error('A204 changed SRS');if(s.navigation.pages['module/A204'].fields.drill1!==old.navigation.pages['module/A204'].fields.drill1)throw Error('Lost A204 old answer');})()`);
 console.log('A204 smoke passed: three units, 23 banks, nested navigation, multiline exams, pending reviews, old archive/SRS, two references and mobile layout.');
}
