...
export async function checkB103({evaluate,route,command,poll,screenshot,delay,importSynthetic}){
 ...
 await evaluate(`(async()=>{
  ...
  s.moduleProgress.B103={selfChecked:true,date:'2026-09-22'};s.drafts.B103='Synthetic original notes for B103.';
  s.cards['B103-v1']=reviewCard(null,'good',Date.UTC(2026,8,22));
  s.navigation.pages['module/B103']={scroll:0,focus:'drill1',fields:{drill0:'sent',drill1:'must\\nOriginal answer',drill2:'The room is cleaned.'},details:[]};
  window.__b103Import=JSON.stringify(s);
 })()`);
 const original=await evaluate('window.__b103Import');await importSynthetic('window.__b103Import');
 await route('module/B103','#legacy-practice');
 ...
 assert.equal(await evaluate('document.querySelector("#draft").value'),'Synthetic original notes for B103.');
 assert.equal(await evaluate('document.querySelector("#drill1").value'),'must\nOriginal answer');
 ...
 await route('home','.hero');await route('module/B103','#legacy-practice');
 await command('Page.reload');await poll(()=>evaluate('document.querySelector("#drill2")?.value==="The room is cleaned."'),'B103 archive reload');
 await screenshot('b103-topic-desktop.png');
 const units=await evaluate(`(async()=>{const {subtopics}=await import('/data/course.mjs');return subtopics.filter(u=>u.topic==='B103')})()`);
 for(const u of units){
  ...
  for(const b of u.banks){
   ...
   if(b.kind==='listening'&&u.id==='B103-focus'){
 ...
 for(const [id,count,query] of [['passive-forms',35,'being'],['service-arrangements',26,'have']]){
  ...
  await poll(()=>evaluate(`document.querySelector('#reference-search')?.value===${JSON.stringify(query)}`),'B103 reference query restored');
  assert.equal(await evaluate('document.querySelectorAll("#reference-rows tr").length'),filtered);
  await fill('#reference-search','');await screenshot('b103-'+id+'-desktop.png');
 }
 await command('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
 for(const [path,selector,name] of [['module/B103','#legacy-practice','topic'],['unit/B103-services/production','#check-bank','writing'],['unit/B103-processes/reading','#check-bank','reading'],['references/passive-forms','#reference-search','reference'],['references/service-arrangements','#reference-search','services-reference']]){
  await route(path,selector);assert(await evaluate('document.documentElement.scrollWidth<=window.innerWidth'),'B103 mobile overflow '+path);
  ...
  await screenshot('b103-'+name+'-mobile.png');
 }
 await evaluate(`(async()=>{const {validateState}=await import('/engine.mjs');const s=JSON.parse(localStorage.getItem('english-training-v1'));validateState(s);const old=JSON.parse(${JSON.stringify(original)});if(JSON.stringify(s.cards)!==JSON.stringify(old.cards))throw Error('B103 changed SRS');if(s.navigation.pages['module/B103'].fields.drill1!==old.navigation.pages['module/B103'].fields.drill1)throw Error('Lost B103 old answer');})()`);
 console.log('B103 smoke passed: three units, 22 banks, nested navigation, multiline exams, pending reviews, old archive/SRS, two references and mobile layout.');
}
