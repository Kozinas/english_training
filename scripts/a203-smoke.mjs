...
export async function checkA203({evaluate,route,command,poll,screenshot,delay,importSynthetic}){
 ...
 await evaluate(`(async()=>{
  ...
  s.moduleProgress.A203={selfChecked:true,date:'2026-09-22'};s.drafts.A203='Synthetic original notes for A203.';
  s.cards['A203-v1']=reviewCard(null,'good',Date.UTC(2026,8,22));
  s.navigation.pages['module/A203']={scroll:0,focus:'drill1',fields:{drill0:'arrives',drill1:'must not\\nOriginal answer',drill2:'do not have to'},details:[]};
  window.__a203Import=JSON.stringify(s);
 })()`);
 const original=await evaluate('window.__a203Import');await importSynthetic('window.__a203Import');
 await route('module/A203','#legacy-practice');
 assert.equal(await evaluate('document.querySelectorAll(".unit-card").length'),3);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").max'),265);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").value'),0,'no inherited new progress');
 assert.equal(await evaluate('document.querySelector("#draft").value'),'Synthetic original notes for A203.');
 assert.equal(await evaluate('document.querySelector("#drill1").value'),'must not\nOriginal answer');
 ...
 await route('home','.hero');await route('module/A203','#legacy-practice');
 await command('Page.reload');await poll(()=>evaluate('document.querySelector("#drill2")?.value==="do not have to"'),'A203 archive reload');
 await screenshot('a203-topic-desktop.png');
 const units=await evaluate(`(async()=>{const {subtopics}=await import('/data/course.mjs');return subtopics.filter(u=>u.topic==='A203')})()`);
 for(const u of units){
  ...
  for(const b of u.banks){
   ...
   if(b.kind==='listening'&&u.id==='A203-obligation'){
 ...
 for(const [id,count,query] of [['rules-conditions',51,'until']]){
  ...
  await poll(()=>evaluate(`document.querySelector('#reference-search')?.value===${JSON.stringify(query)}`),'A203 reference query restored');
  assert.equal(await evaluate('document.querySelectorAll("#reference-rows tr").length'),filtered);
  await fill('#reference-search','');await screenshot('a203-'+id+'-desktop.png');
 }
 await command('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
 for(const [path,selector,name] of [['module/A203','#legacy-practice','topic'],['unit/A203-conditions/production','#check-bank','writing'],['unit/A203-advice/reading','#check-bank','reading'],['references/rules-conditions','#reference-search','reference']]){
  await route(path,selector);assert(await evaluate('document.documentElement.scrollWidth<=window.innerWidth'),'A203 mobile overflow '+path);
  ...
  await screenshot('a203-'+name+'-mobile.png');
 }
 await evaluate(`(async()=>{const {validateState}=await import('/engine.mjs');const s=JSON.parse(localStorage.getItem('english-training-v1'));validateState(s);const old=JSON.parse(${JSON.stringify(original)});if(JSON.stringify(s.cards)!==JSON.stringify(old.cards))throw Error('A203 changed SRS');if(s.navigation.pages['module/A203'].fields.drill1!==old.navigation.pages['module/A203'].fields.drill1)throw Error('Lost A203 old answer');})()`);
 console.log('A203 smoke passed: three units, 22 banks, nested navigation, multiline exams, pending reviews, old archive/SRS, reference and mobile layout.');
}
