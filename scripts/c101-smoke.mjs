...
export async function checkC101({evaluate,route,command,poll,screenshot,delay,importSynthetic}){
 ...
 await evaluate(`(async()=>{
  ...
  s.moduleProgress.C101={selfChecked:true,date:'2026-09-22'};s.drafts.C101='Synthetic original notes for C101.';
  s.cards['C101-v1']=reviewCard(null,'good',Date.UTC(2026,8,22));
  s.navigation.pages['module/C101']={scroll:0,focus:'drill1',fields:{drill0:'do',drill1:'understand\\nOriginal answer',drill2:'is'},details:[]};
  window.__c101Import=JSON.stringify(s);
 })()`);
 const original=await evaluate('window.__c101Import');await importSynthetic('window.__c101Import');
 await route('module/C101','#legacy-practice');
 assert.equal(await evaluate('document.querySelectorAll(".unit-card").length'),3);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").max'),277);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").value'),0,'no inherited new progress');
 assert.equal(await evaluate('document.querySelector("#draft").value'),'Synthetic original notes for C101.');
 assert.equal(await evaluate('document.querySelector("#drill1").value'),'understand\nOriginal answer');
 ...
 await route('home','.hero');await route('module/C101','#legacy-practice');
 await command('Page.reload');await poll(()=>evaluate('document.querySelector("#drill2")?.value==="is"'),'C101 archive reload');
 await screenshot('c101-topic-desktop.png');
 const units=await evaluate(`(async()=>{const {subtopics}=await import('/data/course.mjs');return subtopics.filter(u=>u.topic==='C101')})()`);
 for(const u of units){
  ...
  for(const b of u.banks){
   ...
   if(b.kind==='listening'&&u.id==='C101-inversion'){
 ...
 for(const [id,count,query] of [['inversion-focus',34,'rarely'],['cleft-focus',30,'what'],['fronting-information',28,'guide']]){
  ...
  await poll(()=>evaluate(`document.querySelector('#reference-search')?.value===${JSON.stringify(query)}`),'C101 reference query restored');
  assert.equal(await evaluate('document.querySelectorAll("#reference-rows tr").length'),filtered);
  await fill('#reference-search','');await screenshot('c101-'+id+'-desktop.png');
 }
 await command('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
 for(const [path,selector,name] of [['module/C101','#legacy-practice','topic'],['unit/C101-fronting/production','#check-bank','writing'],['unit/C101-clefts/reading','#check-bank','reading'],['references/inversion-focus','#reference-search','reference'],['references/cleft-focus','#reference-search','argument-reference'],['references/fronting-information','#reference-search','lexical-reference']]){
  await route(path,selector);assert(await evaluate('document.documentElement.scrollWidth<=window.innerWidth'),'C101 mobile overflow '+path);
  ...
  await screenshot('c101-'+name+'-mobile.png');
 }
 await evaluate(`(async()=>{const {validateState}=await import('/engine.mjs');const s=JSON.parse(localStorage.getItem('english-training-v1'));validateState(s);const old=JSON.parse(${JSON.stringify(original)});if(JSON.stringify(s.cards)!==JSON.stringify(old.cards))throw Error('C101 changed SRS');if(s.navigation.pages['module/C101'].fields.drill1!==old.navigation.pages['module/C101'].fields.drill1)throw Error('Lost C101 old answer');})()`);
 console.log('C101 smoke passed: three units, 22 banks, nested navigation, multiline exams, pending reviews, old archive/SRS, three references and mobile layout.');
}
