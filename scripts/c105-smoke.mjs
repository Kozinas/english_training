...
export async function checkC105({evaluate,route,command,poll,screenshot,delay,importSynthetic}){
 ...
 await evaluate(`(async()=>{
  ...
  s.moduleProgress.C105={selfChecked:true,date:'2026-09-22'};s.drafts.C105='Synthetic original notes for C105.';
  s.cards['C105-v1']=reviewCard(null,'good',Date.UTC(2026,8,22));
  s.navigation.pages['module/C105']={scroll:0,focus:'drill1',fields:{drill0:'to',drill1:'no\\nOriginal answer',drill2:'on'},details:[]};
  window.__c105Import=JSON.stringify(s);
 })()`);
 const original=await evaluate('window.__c105Import');await importSynthetic('window.__c105Import');
 await route('module/C105','#legacy-practice');
 assert.equal(await evaluate('document.querySelectorAll(".unit-card").length'),3);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").max'),273);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").value'),0,'no inherited new progress');
 assert.equal(await evaluate('document.querySelector("#draft").value'),'Synthetic original notes for C105.');
 assert.equal(await evaluate('document.querySelector("#drill1").value'),'no\nOriginal answer');
 ...
 await route('home','.hero');await route('module/C105','#legacy-practice');
 await command('Page.reload');await poll(()=>evaluate('document.querySelector("#drill2")?.value==="on"'),'C105 archive reload');
 await screenshot('c105-topic-desktop.png');
 const units=await evaluate(`(async()=>{const {subtopics}=await import('/data/course.mjs');return subtopics.filter(u=>u.topic==='C105')})()`);
 for(const u of units){
  ...
  for(const b of u.banks){
   ...
   if(b.kind==='listening'&&u.id==='C105-sources'){
 ...
 for(const [id,count,query] of [['source-attribution',26,'evidence'],['source-synthesis',26,'group'],['argument-mediation',28,'claim']]){
  ...
  await poll(()=>evaluate(`document.querySelector('#reference-search')?.value===${JSON.stringify(query)}`),'C105 reference query restored');
  assert.equal(await evaluate('document.querySelectorAll("#reference-rows tr").length'),filtered);
  await fill('#reference-search','');await screenshot('c105-'+id+'-desktop.png');
 }
 await command('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
 for(const [path,selector,name] of [['module/C105','#legacy-practice','topic'],['unit/C105-argument/production','#check-bank','writing'],['unit/C105-synthesis/reading','#check-bank','reading'],['references/source-attribution','#reference-search','reference'],['references/source-synthesis','#reference-search','links-reference'],['references/argument-mediation','#reference-search','editing-reference']]){
  await route(path,selector);assert(await evaluate('document.documentElement.scrollWidth<=window.innerWidth'),'C105 mobile overflow '+path);
  ...
  await screenshot('c105-'+name+'-mobile.png');
 }
 await evaluate(`(async()=>{const {validateState}=await import('/engine.mjs');const s=JSON.parse(localStorage.getItem('english-training-v1'));validateState(s);const old=JSON.parse(${JSON.stringify(original)});if(JSON.stringify(s.cards)!==JSON.stringify(old.cards))throw Error('C105 changed SRS');if(s.navigation.pages['module/C105'].fields.drill1!==old.navigation.pages['module/C105'].fields.drill1)throw Error('Lost C105 old answer');})()`);
 console.log('C105 smoke passed: three units, 21 banks, nested navigation, multiline exams, pending reviews, old archive/SRS, three references and mobile layout.');
}
