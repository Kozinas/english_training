...
export async function checkB105({evaluate,route,command,poll,screenshot,delay,importSynthetic}){
 ...
 await evaluate(`(async()=>{
  ...
  s.moduleProgress.B105={selfChecked:true,date:'2026-09-22'};s.drafts.B105='Synthetic original notes for B105.';
  s.cards['B105-v1']=reviewCard(null,'good',Date.UTC(2026,8,22));
  s.navigation.pages['module/B105']={scroll:0,focus:'drill1',fields:{drill0:'make',drill1:'development\\nOriginal answer',drill2:'без артикля'},details:[]};
  window.__b105Import=JSON.stringify(s);
 })()`);
 const original=await evaluate('window.__b105Import');await importSynthetic('window.__b105Import');
 await route('module/B105','#legacy-practice');
 assert.equal(await evaluate('document.querySelectorAll(".unit-card").length'),4);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").max'),360);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").value'),0,'no inherited new progress');
 assert.equal(await evaluate('document.querySelector("#draft").value'),'Synthetic original notes for B105.');
 assert.equal(await evaluate('document.querySelector("#drill1").value'),'development\nOriginal answer');
 ...
 await route('home','.hero');await route('module/B105','#legacy-practice');
 await command('Page.reload');await poll(()=>evaluate('document.querySelector("#drill2")?.value==="без артикля"'),'B105 archive reload');
 await screenshot('b105-topic-desktop.png');
 const units=await evaluate(`(async()=>{const {subtopics}=await import('/data/course.mjs');return subtopics.filter(u=>u.topic==='B105')})()`);
 for(const u of units){
  ...
  for(const b of u.banks){
   ...
   if(b.kind==='listening'&&u.id==='B105-reference'){
 ...
 for(const [id,count,query] of [['text-cohesion',36,'despite'],['word-building',44,'research'],['email-writing',26,'confirm']]){
  ...
  await poll(()=>evaluate(`document.querySelector('#reference-search')?.value===${JSON.stringify(query)}`),'B105 reference query restored');
  assert.equal(await evaluate('document.querySelectorAll("#reference-rows tr").length'),filtered);
  await fill('#reference-search','');await screenshot('b105-'+id+'-desktop.png');
 }
 await command('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
 for(const [path,selector,name] of [['module/B105','#legacy-practice','topic'],['unit/B105-messages/production','#check-bank','writing'],['unit/B105-cohesion/reading','#check-bank','reading'],['references/text-cohesion','#reference-search','reference'],['references/word-building','#reference-search','words-reference'],['references/email-writing','#reference-search','email-reference']]){
  await route(path,selector);assert(await evaluate('document.documentElement.scrollWidth<=window.innerWidth'),'B105 mobile overflow '+path);
  ...
  await screenshot('b105-'+name+'-mobile.png');
 }
 await evaluate(`(async()=>{const {validateState}=await import('/engine.mjs');const s=JSON.parse(localStorage.getItem('english-training-v1'));validateState(s);const old=JSON.parse(${JSON.stringify(original)});if(JSON.stringify(s.cards)!==JSON.stringify(old.cards))throw Error('B105 changed SRS');if(s.navigation.pages['module/B105'].fields.drill1!==old.navigation.pages['module/B105'].fields.drill1)throw Error('Lost B105 old answer');})()`);
 console.log('B105 smoke passed: four units, 30 banks, nested navigation, multiline exams, pending reviews, old archive/SRS, three references and mobile layout.');
}
