...
export async function checkC202({evaluate,route,command,poll,screenshot,delay,importSynthetic}){
 ...
 await evaluate(`(async()=>{
  ...
  s.moduleProgress.C202={selfChecked:true,date:'2026-09-22'};s.drafts.C202='Synthetic original notes for C202.';
  s.cards['C202-v1']=reviewCard(null,'good',Date.UTC(2026,8,22));
  s.navigation.pages['module/C202']={scroll:0,focus:'drill1',fields:{drill0:'heavy',drill1:'explicit\\nOriginal answer',drill2:'for'},details:[]};
  window.__c202Import=JSON.stringify(s);
 })()`);
 const original=await evaluate('window.__c202Import');await importSynthetic('window.__c202Import');
 await route('module/C202','#legacy-practice');
 assert.equal(await evaluate('document.querySelectorAll(".unit-card").length'),3);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").max'),275);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").value'),0,'no inherited new progress');
 assert.equal(await evaluate('document.querySelector("#draft").value'),'Synthetic original notes for C202.');
 assert.equal(await evaluate('document.querySelector("#drill1").value'),'explicit\nOriginal answer');
 ...
 await route('home','.hero');await route('module/C202','#legacy-practice');
 await command('Page.reload');await poll(()=>evaluate('document.querySelector("#drill2")?.value==="for"'),'C202 archive reload');
 await screenshot('c202-topic-desktop.png');
 const units=await evaluate(`(async()=>{const {subtopics}=await import('/data/course.mjs');return subtopics.filter(u=>u.topic==='C202')})()`);
 for(const u of units){
  ...
  for(const b of u.banks){
   ...
   if(b.kind==='listening'&&u.id==='C202-idiomatic'){
 ...
 for(const [id,count,query] of [['idiomatic-style',24,'metaphor'],['sentence-economy',26,'passive'],['revision-workflow',26,'voice']]){
  ...
  await poll(()=>evaluate(`document.querySelector('#reference-search')?.value===${JSON.stringify(query)}`),'C202 reference query restored');
  assert.equal(await evaluate('document.querySelectorAll("#reference-rows tr").length'),filtered);
  await fill('#reference-search','');await screenshot('c202-'+id+'-desktop.png');
 }
 await command('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
 for(const [path,selector,name] of [['module/C202','#legacy-practice','topic'],['unit/C202-revision/production','#check-bank','writing'],['unit/C202-concision/reading','#check-bank','reading'],['references/idiomatic-style','#reference-search','reference'],['references/sentence-economy','#reference-search','links-reference'],['references/revision-workflow','#reference-search','editing-reference']]){
  await route(path,selector);assert(await evaluate('document.documentElement.scrollWidth<=window.innerWidth'),'C202 mobile overflow '+path);
  ...
  await screenshot('c202-'+name+'-mobile.png');
 }
 await evaluate(`(async()=>{const {validateState}=await import('/engine.mjs');const s=JSON.parse(localStorage.getItem('english-training-v1'));validateState(s);const old=JSON.parse(${JSON.stringify(original)});if(JSON.stringify(s.cards)!==JSON.stringify(old.cards))throw Error('C202 changed SRS');if(s.navigation.pages['module/C202'].fields.drill1!==old.navigation.pages['module/C202'].fields.drill1)throw Error('Lost C202 old answer');})()`);
 console.log('C202 smoke passed: three units, 21 banks, nested navigation, multiline exams, pending reviews, old archive/SRS, three references and mobile layout.');
}
