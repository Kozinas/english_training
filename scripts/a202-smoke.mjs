...
export async function checkA202({evaluate,route,command,poll,screenshot,delay,importSynthetic}){
 ...
 await evaluate(`(async()=>{
  ...
  s.moduleProgress.A202={selfChecked:true,date:'2026-09-22'};s.drafts.A202='Synthetic original notes for A202.';
  s.cards['A202-v1']=reviewCard(null,'good',Date.UTC(2026,8,22));
  s.navigation.pages['module/A202']={scroll:0,focus:'drill1',fields:{drill0:'better',drill1:'carefully\\nOriginal answer',drill2:'enough'},details:[]};
  window.__a202Import=JSON.stringify(s);
 })()`);
 const original=await evaluate('window.__a202Import');await importSynthetic('window.__a202Import');
 await route('module/A202','#legacy-practice');
 assert.equal(await evaluate('document.querySelectorAll(".unit-card").length'),3);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").max'),253);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").value'),0,'no inherited new progress');
 assert.equal(await evaluate('document.querySelector("#draft").value'),'Synthetic original notes for A202.');
 assert.equal(await evaluate('document.querySelector("#drill1").value'),'carefully\nOriginal answer');
 ...
 await route('home','.hero');await route('module/A202','#legacy-practice');
 await command('Page.reload');await poll(()=>evaluate('document.querySelector("#drill2")?.value==="enough"'),'A202 archive reload');
 await screenshot('a202-topic-desktop.png');
 const units=await evaluate(`(async()=>{const {subtopics}=await import('/data/course.mjs');return subtopics.filter(u=>u.topic==='A202')})()`);
 for(const u of units){
  ...
  for(const b of u.banks){
   ...
   if(b.kind==='listening'&&u.id==='A202-description'){
 ...
 for(const [id,count,query] of [['comparison',56,'enough']]){
  ...
  await poll(()=>evaluate(`document.querySelector('#reference-search')?.value===${JSON.stringify(query)}`),'A202 reference query restored');
  assert.equal(await evaluate('document.querySelectorAll("#reference-rows tr").length'),filtered);
  await fill('#reference-search','');await screenshot('a202-'+id+'-desktop.png');
 }
 await command('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
 for(const [path,selector,name] of [['module/A202','#legacy-practice','topic'],['unit/A202-degree/production','#check-bank','writing'],['unit/A202-comparison/reading','#check-bank','reading'],['references/comparison','#reference-search','reference']]){
  await route(path,selector);assert(await evaluate('document.documentElement.scrollWidth<=window.innerWidth'),'A202 mobile overflow '+path);
  ...
  await screenshot('a202-'+name+'-mobile.png');
 }
 await evaluate(`(async()=>{const {validateState}=await import('/engine.mjs');const s=JSON.parse(localStorage.getItem('english-training-v1'));validateState(s);const old=JSON.parse(${JSON.stringify(original)});if(JSON.stringify(s.cards)!==JSON.stringify(old.cards))throw Error('A202 changed SRS');if(s.navigation.pages['module/A202'].fields.drill1!==old.navigation.pages['module/A202'].fields.drill1)throw Error('Lost A202 old answer');})()`);
 console.log('A202 smoke passed: three units, 21 banks, nested navigation, multiline exams, pending reviews, old archive/SRS, reference and mobile layout.');
}
