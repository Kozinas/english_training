...
export async function checkB101({evaluate,route,command,poll,screenshot,delay,importSynthetic}){
 ...
 await evaluate(`(async()=>{
  ...
  s.moduleProgress.B101={selfChecked:true,date:'2026-09-22'};s.drafts.B101='Synthetic original notes for B101.';
  s.cards['B101-v1']=reviewCard(null,'good',Date.UTC(2026,8,22));
  s.navigation.pages['module/B101']={scroll:0,focus:'drill1',fields:{drill0:'waiting',drill1:'had\\nOriginal answer',drill2:'getting up'},details:[]};
  window.__b101Import=JSON.stringify(s);
 })()`);
 const original=await evaluate('window.__b101Import');await importSynthetic('window.__b101Import');
 await route('module/B101','#legacy-practice');
 assert.equal(await evaluate('document.querySelectorAll(".unit-card").length'),3);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").max'),301);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").value'),0,'no inherited new progress');
 assert.equal(await evaluate('document.querySelector("#draft").value'),'Synthetic original notes for B101.');
 assert.equal(await evaluate('document.querySelector("#drill1").value'),'had\nOriginal answer');
 ...
 await route('home','.hero');await route('module/B101','#legacy-practice');
 await command('Page.reload');await poll(()=>evaluate('document.querySelector("#drill2")?.value==="getting up"'),'B101 archive reload');
 await screenshot('b101-topic-desktop.png');
 const units=await evaluate(`(async()=>{const {subtopics}=await import('/data/course.mjs');return subtopics.filter(u=>u.topic==='B101')})()`);
 for(const u of units){
  ...
  for(const b of u.banks){
   ...
   if(b.kind==='listening'&&u.id==='B101-continuous'){
 ...
 for(const [id,count,query] of [['perfect-continuous',25,'since'],['past-perfect',27,'before'],['habits-adaptation',24,'would']]){
  ...
  await poll(()=>evaluate(`document.querySelector('#reference-search')?.value===${JSON.stringify(query)}`),'B101 reference query restored');
  assert.equal(await evaluate('document.querySelectorAll("#reference-rows tr").length'),filtered);
  await fill('#reference-search','');await screenshot('b101-'+id+'-desktop.png');
 }
 await command('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
 for(const [path,selector,name] of [['module/B101','#legacy-practice','topic'],['unit/B101-habits/production','#check-bank','writing'],['unit/B101-earlier/reading','#check-bank','reading'],['references/perfect-continuous','#reference-search','reference'],['references/past-perfect','#reference-search','phrasal-reference'],['references/habits-adaptation','#reference-search','time-reference']]){
  await route(path,selector);assert(await evaluate('document.documentElement.scrollWidth<=window.innerWidth'),'B101 mobile overflow '+path);
  ...
  await screenshot('b101-'+name+'-mobile.png');
 }
 await evaluate(`(async()=>{const {validateState}=await import('/engine.mjs');const s=JSON.parse(localStorage.getItem('english-training-v1'));validateState(s);const old=JSON.parse(${JSON.stringify(original)});if(JSON.stringify(s.cards)!==JSON.stringify(old.cards))throw Error('B101 changed SRS');if(s.navigation.pages['module/B101'].fields.drill1!==old.navigation.pages['module/B101'].fields.drill1)throw Error('Lost B101 old answer');})()`);
 console.log('B101 smoke passed: three units, 22 banks, nested navigation, multiline exams, pending reviews, old archive/SRS, three references and mobile layout.');
}
