...
export async function checkB202({evaluate,route,command,poll,screenshot,delay,importSynthetic}){
 ...
 await evaluate(`(async()=>{
  ...
  s.moduleProgress.B202={selfChecked:true,date:'2026-09-22'};s.drafts.B202='Synthetic original notes for B202.';
  s.cards['B202-v1']=reviewCard(null,'good',Date.UTC(2026,8,22));
  s.navigation.pages['module/B202']={scroll:0,focus:'drill1',fields:{drill0:'would',drill1:'had\\nOriginal answer',drill2:'left'},details:[]};
  window.__b202Import=JSON.stringify(s);
 })()`);
 const original=await evaluate('window.__b202Import');await importSynthetic('window.__b202Import');
 await route('module/B202','#legacy-practice');
 ...
 assert.equal(await evaluate('document.querySelector("#draft").value'),'Synthetic original notes for B202.');
 ...
 await route('home','.hero');await route('module/B202','#legacy-practice');
 await command('Page.reload');await poll(()=>evaluate('document.querySelector("#drill2")?.value==="left"'),'B202 archive reload');
 await screenshot('b202-topic-desktop.png');
 const units=await evaluate(`(async()=>{const {subtopics}=await import('/data/course.mjs');return subtopics.filter(u=>u.topic==='B202')})()`);
 for(const u of units){
  ...
  for(const b of u.banks){
   ...
   if(b.kind==='listening'&&u.id==='B202-alternatives'){
 ...
 for(const [id,count,query] of [['counterfactuals',32,'now'],['past-modal-choices',38,'need']]){
  ...
  await poll(()=>evaluate(`document.querySelector('#reference-search')?.value===${JSON.stringify(query)}`),'B202 reference query restored');
  assert.equal(await evaluate('document.querySelectorAll("#reference-rows tr").length'),filtered);
  await fill('#reference-search','');await screenshot('b202-'+id+'-desktop.png');
 }
 await command('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
 for(const [path,selector,name] of [['module/B202','#legacy-practice','topic'],['unit/B202-regrets/production','#check-bank','writing'],['unit/B202-deduction/reading','#check-bank','reading'],['references/counterfactuals','#reference-search','reference'],['references/past-modal-choices','#reference-search','modal-reference']]){
  await route(path,selector);assert(await evaluate('document.documentElement.scrollWidth<=window.innerWidth'),'B202 mobile overflow '+path);
  ...
  await screenshot('b202-'+name+'-mobile.png');
 }
 await evaluate(`(async()=>{const {validateState}=await import('/engine.mjs');const s=JSON.parse(localStorage.getItem('english-training-v1'));validateState(s);const old=JSON.parse(${JSON.stringify(original)});if(JSON.stringify(s.cards)!==JSON.stringify(old.cards))throw Error('B202 changed SRS');if(s.navigation.pages['module/B202'].fields.drill1!==old.navigation.pages['module/B202'].fields.drill1)throw Error('Lost B202 old answer');})()`);
 console.log('B202 smoke passed: three units, 22 banks, nested navigation, multiline exams, pending reviews, old archive/SRS, two references and mobile layout.');
}
