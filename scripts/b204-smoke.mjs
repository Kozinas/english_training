...
export async function checkB204({evaluate,route,command,poll,screenshot,delay,importSynthetic}){
 ...
 await evaluate(`(async()=>{
  ...
  s.moduleProgress.B204={selfChecked:true,date:'2026-09-22'};s.drafts.B204='Synthetic original notes for B204.';
  s.cards['B204-v1']=reviewCard(null,'good',Date.UTC(2026,8,22));
  s.navigation.pages['module/B204']={scroll:0,focus:'drill1',fields:{drill0:'Despite',drill1:'фактический\\nOriginal answer',drill2:'into'},details:[]};
  window.__b204Import=JSON.stringify(s);
 })()`);
 const original=await evaluate('window.__b204Import');await importSynthetic('window.__b204Import');
 await route('module/B204','#legacy-practice');
 assert.equal(await evaluate('document.querySelectorAll(".unit-card").length'),3);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").max'),263);
 assert.equal(await evaluate('document.querySelector(".topic-progress progress").value'),0,'no inherited new progress');
 assert.equal(await evaluate('document.querySelector("#draft").value'),'Synthetic original notes for B204.');
 assert.equal(await evaluate('document.querySelector("#drill1").value'),'фактический\nOriginal answer');
 ...
 await route('home','.hero');await route('module/B204','#legacy-practice');
 await command('Page.reload');await poll(()=>evaluate('document.querySelector("#drill2")?.value==="into"'),'B204 archive reload');
 await screenshot('b204-topic-desktop.png');
 const units=await evaluate(`(async()=>{const {subtopics}=await import('/data/course.mjs');return subtopics.filter(u=>u.topic==='B204')})()`);
 for(const u of units){
  ...
  for(const b of u.banks){
   ...
   if(b.kind==='listening'&&u.id==='B204-register'){
 ...
 for(const [id,count,query] of [['register-hedging',28,'request'],['argument-concession',32,'however'],['lexical-precision',36,'explain']]){
  ...
  await poll(()=>evaluate(`document.querySelector('#reference-search')?.value===${JSON.stringify(query)}`),'B204 reference query restored');
  assert.equal(await evaluate('document.querySelectorAll("#reference-rows tr").length'),filtered);
  await fill('#reference-search','');await screenshot('b204-'+id+'-desktop.png');
 }
 await command('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
 for(const [path,selector,name] of [['module/B204','#legacy-practice','topic'],['unit/B204-lexis/production','#check-bank','writing'],['unit/B204-argument/reading','#check-bank','reading'],['references/register-hedging','#reference-search','reference'],['references/argument-concession','#reference-search','argument-reference'],['references/lexical-precision','#reference-search','lexical-reference']]){
  await route(path,selector);assert(await evaluate('document.documentElement.scrollWidth<=window.innerWidth'),'B204 mobile overflow '+path);
  ...
  await screenshot('b204-'+name+'-mobile.png');
 }
 await evaluate(`(async()=>{const {validateState}=await import('/engine.mjs');const s=JSON.parse(localStorage.getItem('english-training-v1'));validateState(s);const old=JSON.parse(${JSON.stringify(original)});if(JSON.stringify(s.cards)!==JSON.stringify(old.cards))throw Error('B204 changed SRS');if(s.navigation.pages['module/B204'].fields.drill1!==old.navigation.pages['module/B204'].fields.drill1)throw Error('Lost B204 old answer');})()`);
 console.log('B204 smoke passed: three units, 21 banks, nested navigation, multiline exams, pending reviews, old archive/SRS, three references and mobile layout.');
}
