...
export async function checkC103({evaluate,route,command,poll,screenshot,delay,importSynthetic}){
 ...
 await evaluate(`(async()=>{
  ...
  s.moduleProgress.C103={selfChecked:true,date:'2026-09-22'};s.drafts.C103='Synthetic original notes for C103.';
  s.cards['C103-v1']=reviewCard(null,'good',Date.UTC(2026,8,22));
  s.navigation.pages['module/C103']={scroll:0,focus:'drill1',fields:{drill0:'implementation',drill1:'reporting\\nOriginal answer',drill2:'approval'},details:[]};
  window.__c103Import=JSON.stringify(s);
 })()`);
 const original=await evaluate('window.__c103Import');await importSynthetic('window.__c103Import');
 await route('module/C103','#legacy-practice');
 ...
 assert.equal(await evaluate('document.querySelector("#draft").value'),'Synthetic original notes for C103.');
 assert.equal(await evaluate('document.querySelector("#drill1").value'),'reporting\nOriginal answer');
 ...
 await route('home','.hero');await route('module/C103','#legacy-practice');
 await command('Page.reload');await poll(()=>evaluate('document.querySelector("#drill2")?.value==="approval"'),'C103 archive reload');
 await screenshot('c103-topic-desktop.png');
 const units=await evaluate(`(async()=>{const {subtopics}=await import('/data/course.mjs');return subtopics.filter(u=>u.topic==='C103')})()`);
 for(const u of units){
  ...
  for(const b of u.banks){
   ...
   if(b.kind==='listening'&&u.id==='C103-nouns'){
 ...
 for(const [id,count,query] of [['nominal-groups',28,'review'],['reference-chains',24,'that'],['meaning-preserving-editing',24,'may']]){
  ...
  await poll(()=>evaluate(`document.querySelector('#reference-search')?.value===${JSON.stringify(query)}`),'C103 reference query restored');
  assert.equal(await evaluate('document.querySelectorAll("#reference-rows tr").length'),filtered);
  await fill('#reference-search','');await screenshot('c103-'+id+'-desktop.png');
 }
 await command('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});
 for(const [path,selector,name] of [['module/C103','#legacy-practice','topic'],['unit/C103-editing/production','#check-bank','writing'],['unit/C103-links/reading','#check-bank','reading'],['references/nominal-groups','#reference-search','reference'],['references/reference-chains','#reference-search','links-reference'],['references/meaning-preserving-editing','#reference-search','editing-reference']]){
  await route(path,selector);assert(await evaluate('document.documentElement.scrollWidth<=window.innerWidth'),'C103 mobile overflow '+path);
  ...
  await screenshot('c103-'+name+'-mobile.png');
 }
 await evaluate(`(async()=>{const {validateState}=await import('/engine.mjs');const s=JSON.parse(localStorage.getItem('english-training-v1'));validateState(s);const old=JSON.parse(${JSON.stringify(original)});if(JSON.stringify(s.cards)!==JSON.stringify(old.cards))throw Error('C103 changed SRS');if(s.navigation.pages['module/C103'].fields.drill1!==old.navigation.pages['module/C103'].fields.drill1)throw Error('Lost C103 old answer');})()`);
 console.log('C103 smoke passed: three units, 21 banks, nested navigation, multiline exams, pending reviews, old archive/SRS, three references and mobile layout.');
}
