...
test('legacy topics have an explicitly separate binary self-check indicator',()=>{
 const state=freshState();state.drafts.C204='A long draft does not prove a check.';
 assert.deepEqual(topicWorkProgress(state,'C204'),{kind:'legacy',completed:0,total:1,percent:0});
 state.moduleProgress.C204={selfChecked:true,date:'2026-09-22'};
 assert.deepEqual(topicWorkProgress(state,'C204'),{kind:'legacy',completed:1,total:1,percent:100});
 state.moduleProgress.C204.selfChecked=false;
 assert.equal(topicWorkProgress(state,'C204').percent,0);
});
...
test('v1 migration retains old marks but does not apply them to expanded topics',()=>{
 ...
 old.moduleProgress={P01:{selfChecked:true,date:'2026-09-22'},A104:{selfChecked:true,date:'2026-09-22'},A105:{selfChecked:true,date:'2026-09-22'},A201:{selfChecked:true,date:'2026-09-22'},A202:{selfChecked:true,date:'2026-09-22'},A203:{selfChecked:true,date:'2026-09-22'},A204:{selfChecked:true,date:'2026-09-22'},A205:{selfChecked:true,date:'2026-09-22'}};
 ...
 old.moduleProgress.B105={selfChecked:true,date:'2026-09-22'};
 old.moduleProgress.B201={selfChecked:true,date:'2026-09-22'};
 old.moduleProgress.B202={selfChecked:true,date:'2026-09-22'};
 old.moduleProgress.B203={selfChecked:true,date:'2026-09-22'};
 old.moduleProgress.B204={selfChecked:true,date:'2026-09-22'};
 old.moduleProgress.B205={selfChecked:true,date:'2026-09-22'};
 old.moduleProgress.C101={selfChecked:true,date:'2026-09-22'};
 old.moduleProgress.C102={selfChecked:true,date:'2026-09-22'};
 old.moduleProgress.C103={selfChecked:true,date:'2026-09-22'};
 old.moduleProgress.C104={selfChecked:true,date:'2026-09-22'};
 old.moduleProgress.C105={selfChecked:true,date:'2026-09-22'};
 old.moduleProgress.C201={selfChecked:true,date:'2026-09-22'};
 old.moduleProgress.C202={selfChecked:true,date:'2026-09-22'};
 old.moduleProgress.C203={selfChecked:true,date:'2026-09-22'};
 old.moduleProgress.C204={selfChecked:true,date:'2026-09-22'};
 ...
 assert.equal(topicWorkProgress(restored,'A105').percent,0);
 assert.equal(topicWorkProgress(restored,'A201').percent,0);
 assert.equal(topicWorkProgress(restored,'A202').percent,0);
 assert.equal(topicWorkProgress(restored,'A203').percent,0);
 assert.equal(topicWorkProgress(restored,'A204').percent,0);
 ...
 assert.equal(topicWorkProgress(restored,'B104').percent,0);
 assert.equal(topicWorkProgress(restored,'B105').percent,0);
 assert.equal(topicWorkProgress(restored,'B201').percent,0);
 assert.equal(topicWorkProgress(restored,'B202').percent,0);
 assert.equal(topicWorkProgress(restored,'B203').percent,0);
 assert.equal(topicWorkProgress(restored,'B204').percent,0);
 assert.equal(topicWorkProgress(restored,'B205').percent,0);
 assert.equal(topicWorkProgress(restored,'C101').percent,0);
 assert.equal(topicWorkProgress(restored,'C102').percent,0);
 assert.equal(topicWorkProgress(restored,'C103').percent,0);
 assert.equal(topicWorkProgress(restored,'C104').percent,0);
 assert.equal(topicWorkProgress(restored,'C105').percent,0);
 assert.equal(topicWorkProgress(restored,'C201').percent,0);
 assert.equal(topicWorkProgress(restored,'C202').percent,0);
 assert.equal(topicWorkProgress(restored,'C203').percent,0);
 assert.equal(topicWorkProgress(restored,'C204').percent,100);
