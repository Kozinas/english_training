...
test('legacy topics have an explicitly separate binary self-check indicator',()=>{
 const state=freshState();state.drafts.A203='A long draft does not prove a check.';
 assert.deepEqual(topicWorkProgress(state,'A203'),{kind:'legacy',completed:0,total:1,percent:0});
 state.moduleProgress.A203={selfChecked:true,date:'2026-09-22'};
 assert.deepEqual(topicWorkProgress(state,'A203'),{kind:'legacy',completed:1,total:1,percent:100});
 state.moduleProgress.A203.selfChecked=false;
 assert.equal(topicWorkProgress(state,'A203').percent,0);
});
...
test('v1 migration retains old marks but does not apply them to expanded topics',()=>{
 ...
 old.moduleProgress={P01:{selfChecked:true,date:'2026-09-22'},A104:{selfChecked:true,date:'2026-09-22'},A105:{selfChecked:true,date:'2026-09-22'},A201:{selfChecked:true,date:'2026-09-22'},A202:{selfChecked:true,date:'2026-09-22'},A203:{selfChecked:true,date:'2026-09-22'}};
 ...
 assert.equal(topicWorkProgress(restored,'A203').percent,100);
