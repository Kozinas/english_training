...
test('self-checked legacy modules leave queue; missing prerequisites are explicit checks',()=>{
  const state=placed(answers());state.moduleProgress.C204={selfChecked:true,date:new Date().toISOString()};
  const plan=buildPlan(state);assert(!plan.items.some(m=>m.id==='C204'));assert(plan.items.some(m=>m.checks.length));
});
